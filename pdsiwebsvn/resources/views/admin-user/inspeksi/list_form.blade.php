...
<script>
    ...
    function jscari(){
        ...
        window.open('/admin-user/inspeksi-list/'+$('#statusnya option:selected').val(),'_self');
    }
</script>
        ...
        <tbody>
            ...
            @foreach($listdata as $data)
                <tr>
                        ...
                        <td>{{ $data->ispeksi_no }}</td>
                        <td>{{ date('Y-m-d',strtotime($data->inspeksi_tgl)) }}</td>
                        <td>{{ $data->form_sts_desc }}</td>
                        <td>
                            <a href="{{ URL::to('/') }}/admin-user/look-inspeksi/{{ $data->routingslip_no }}"><i title="detail" class="fa fa-search" aria-hidden="true"></i></a>
                            <a <?php echo $data->form_sts_desc=='1-Approve'?" style='display:none;' ":"";?> href="{{ URL::to('/') }}/admin-user/edit-inspeksi/{{ $data->routingslip_no }}"><i title="edit" class="fa fa-edit" aria-hidden="true"></i></a>
                            <a <?php echo $data->form_sts_desc=='1-Approve' || $data->ispeksi_no==''?" style='display:none;' ":"";?> onclick="return confirm('Yakin Approve data ?');" href="{{ URL::to('/') }}/admin-user/approve-inspeksi/{{ $data->routingslip_no }}"><i title="approve" class="fa fa-check-square" aria-hidden="true"></i></a>
