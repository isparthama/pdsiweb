...
<script>
    ...
    function jscari(){
        ...
        window.open('/admin-user/realisasi-list/'+$('#statusnya option:selected').val(),'_self');
    }
</script>
        ...
        <tbody>
            ...
            @foreach($listdata as $data)
                <tr>
                        ...
                        <td>
                            <a href="{{ URL::to('/') }}/admin-user/look-realisasi/{{ $data->routingslip_no }}"><i title="edit" class="fa fa-search" aria-hidden="true"></i></a>
                            <a <?php echo $data->form_sts_desc=='1-Approve'?" style='display:none;' ":"";?> href="{{ URL::to('/') }}/admin-user/edit-realisasi/{{ $data->routingslip_no }}"><i title="edit" class="fa fa-edit" aria-hidden="true"></i></a>
                            <a <?php echo $data->form_sts_desc=='1-Approve'?" style='display:none;' ":"";?> onclick="return confirm('Yakin Approve data ?');" href="{{ URL::to('/') }}/admin-user/approve-realisasi/{{ $data->routingslip_no }}"><i title="approve" class="fa fa-check-square" aria-hidden="true"></i></a>
