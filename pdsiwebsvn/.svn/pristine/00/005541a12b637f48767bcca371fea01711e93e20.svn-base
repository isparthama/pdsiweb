...
<script>
    ...
    function jscari(){
        ...
        window.open('/admin-user/pengerjaanulang-list/'+$('#statusnya option:selected').val(),'_self');
    }
</script>
        ...
        <tbody>
            ...
            @foreach($listdata as $data)
                <tr>
                        ...
                        <td>{{ $data->ispeksi_test_no }}</td>
                        <td>{{ date('Y-m-d',strtotime($data->ispeksi_test_tgl)) }}</td>
                        <td>{{ $data->form_sts_desc }}</td>
                        <td>
                            <a  href="{{ URL::to('/') }}/admin-user/edit-pengerjaanulang/{{ $data->routingslip_no }}"><i title="detail" class="fa fa-search" aria-hidden="true"></i></a>
                            <a <?php  echo $data->form_sts_desc=='1-Approve'?" style='display:none;' ":"";?> onclick="return confirm('Yakin Approve data ?');" href="{{ URL::to('/') }}/admin-user/approve-pengerjaanulang/{{ $data->routingslip_no }}"><i title="approve" class="fa fa-check-square" aria-hidden="true"></i></a>
