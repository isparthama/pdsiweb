...
<script>
    ...
    function jscari(){
        ...
        window.open('/admin-user/serahterimakeluar-list/'+$('#statusnya option:selected').val(),'_self');
    }
</script>
...
<table id="example" class="table table-striped table-bordered display" style="width:100%">
        <thead>
                ...
                <tr>
                        <th>No</th>
                        <th>Nomor<br>Serah Terima Keluar</th>
                        <th>Tanggal</th>
        ...
        <tbody>
            ...
            @foreach($listdata as $data)
                <tr>
                        <td>{{ $no++ }}</td>
                        <td>{{ $data->kirim_no }}</a></td>
                        <td>{{ date('Y-m-d',strtotime($data->kirim_tgl)) }}</td>
                        ...
                        <td>
                            <a <?php echo $data->form_sts_desc=='1-Approve'?" style='display:none;' ":"";?> href="{{ URL::to('/') }}/admin-user/serahterimakeluar-edit/{{ $data->routingslip_no }}"><i title="edit" class="fa fa-edit" aria-hidden="true"></i></a>
                            
                            
                            <a <?php echo $data->form_sts_desc=='1-Approve'?" style='display:none;' ":"";?> onclick="<?php echo $data->serahterimakeluar_filename==""?"alert('Verifikasi Data Anda !');return false":"return confirm('Yakin Approve data ?');"?>" href="{{ URL::to('/') }}/admin-user/serahterimakeluar-approve/{{ $data->routingslip_no }}"><i title="approve" class="fa fa-check-square" aria-hidden="true"></i></a>
